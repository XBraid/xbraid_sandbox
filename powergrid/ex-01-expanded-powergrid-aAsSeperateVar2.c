        ...
        braid_StepStatus status)
{
   ...
   /* Store the level and iter for this Step */
   u->level = level;
   u->iter = iter;
   ...
   /* Propagate nonlinear PDE coefficient */
   if (u->value >= 2.0) 
   {
      u->coeff = -1.0;
      //u->value = 2.0 - (u->value - 2.0);
   }
   else if (u->value <= 1.0)
   {
      u->coeff = 1.0;
      //u->value = 1.0 + (1.0 - u->value);
   }
   /* else // Do nothing, d(coeff) /dt = 0
   {
      u->coeff = u->coeff
   } */ 

   /* Propagate PDE solution with backward Euler */
   (u->value) = (1./(1. + (-u->coeff)*(tstop-tstart)))*u->value;


   /* 
    * Correct if you've under or over shot, making sure to check that you've
    * crossed the threshold with this step (i.e., check uprev and u->value).
    */
   if ( (u->value > 2.0) && (uprev <= 2.0))
   {
      /* Take time step until u = 2, then step down */
      dtsmall = (tstop-tstart) - (1.0 - uprev/2.0)/(u->coeff);
      
    //if(dtsmall < 0.0){
    //    printf("HEREEREEEEE 1111  %1.2e   %1.2e   %1.2e   %1.2e   %1.2e   %1.2e    %d    %d\n", dtsmall, tstop, tstart, uprev, u->value, u->coeff, level, istart);
    //}
    //if((1. + (-u->coeff)*dtsmall) < 0){
    //    printf("HEREEREEEEE 3333  %1.2e   %1.2e   %1.2e   %1.2e   %1.2e   %1.2e    %d    %d\n", dtsmall, tstop, tstart, uprev, u->value, u->coeff, level, istart);
    //}
    //
    //printf("HEREEREEEEE 7777  %1.2e   %1.2e   %1.2e   %1.2e   %1.2e    %d    %d\n", tstop, tstart, uprev, u->value, u->coeff, level, istart);
    //printf("                          %1.2e   %1.2e   %1.16e\n", dtsmall, (tstop-tstart), (1./(1. + dtsmall))*2.0);

      u->coeff = -1.0;
      (u->value) = (1./(1. + (-u->coeff)*dtsmall))*2.0;
   }
   else if ((u->value < 1.0) && (uprev >= 1.0))
   {
      /* Take time step until u = 1, then step up */
      dtsmall = (tstop-tstart) - (1.0 - uprev)/(u->coeff);
      
      u->coeff = 1.0;
      (u->value) = (1./(1. + (-u->coeff)*dtsmall))*1.0;   
      
    //if(dtsmall < 0.0){
    //   printf("HEREEREEEEE 2222  %1.23e   %d    %d\n", dtsmall, level, istart);
    //}
    //if((1. + (-u->coeff)*dtsmall) < 0){
    //    printf("HEREEREEEEE 4444  %1.2e   %1.2e   %1.2e   %1.2e   %1.2e   %1.2e    %d    %d\n", dtsmall, tstop, tstart, uprev, u->value, u->coeff, level, istart);
    //}
   }
 //else if( (u->value < 1.0) || (u->value > 2.0) || (uprev > 2.0) || (uprev < 1.0) )
 //{
 //       printf("HEREEREEEEE 5555  %1.2e   %1.2e   %1.16e   %1.2e   %1.2e    %d    %d\n", tstop, tstart, uprev, u->value, u->coeff, level, istart);
 //}
 //else
 //{
 //       printf("---               %1.2e   %1.2e   %1.2e   %1.2e   %1.2e    %d    %d\n", tstop, tstart, uprev, u->value, u->coeff, level, istart);
 //}
...
my_Residual(braid_App        app,
            braid_Vector     ustop,    /* u at tstop */
            braid_Vector     r,        /* u at tstart, but upon exit should hold the residual */
            braid_StepStatus status)
{
   double tstart;             /* current time */
   double tstop;              /* evolve to this time*/
   ...
   (r->value) = (ustop->value) - (1.0/(1. + (-ustop->coeff)*(tstop-tstart)))*(r->value);

   /* Just compute  -Step(r) + ustop */
   //my_Step(app, NULL, NULL, r, status);
   //r->value = -r->value + ustop->value;
   r->coeff = -r->coeff + ustop->coeff;
       ...
       braid_Vector  y)
{
   ...
   (y->coeff) = alpha*(x->coeff) + beta*(y->coeff);
               ...
               double       *norm_ptr)
{
   ...
   dot = (u->value)*(u->value) + (u->coeff)*(u->coeff);
   *norm_ptr = sqrt(dot);
   ...
   while (arg_index < argc)
   {
      if ( strcmp(argv[arg_index], "-help") == 0 )
      {
         if ( rank == 0 )
         {
            ...
            printf("  -tstop <tstop>    : set final time\n");
