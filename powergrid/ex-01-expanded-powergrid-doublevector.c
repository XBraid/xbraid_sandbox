...
{
   double value1;  // PDE solution value
   double coeff1;  // PDE coefficient
   double value2;  // PDE solution value
   double coeff2;  // PDE coefficient
   int    iter, level;
} my_Vector;
        ...
        braid_StepStatus status)
{
   ...
   if (fstop != NULL)
   {
      (u->value1) += (fstop->value1);
      (u->value2) += (fstop->value2);
   }
   
   /* Compute nonlinear PDE coefficient for first value */
   if (u->value2 >= 2.0) 
   {
      u->coeff1 = -1.0;
   }
   else if (u->value2 <= 1.0)
   {
      u->coeff1 = 1.0;
   }
   else if( (u->value2 - u->value1) > 0)
   {
      u->coeff1 = 1.0;
   }
   else
   ...
   /* Use backward Euler to propagate solution to first value*/
   (u->value1) = 1./(1. + (-u->coeff1)*dt)*(u->value2);


   /* Now, compute nonlinear PDE coefficient for second value */
   if (u->value1 >= 2.0) 
   {
      u->coeff2 = -1.0;
   }
   else if (u->value1 <= 1.0)
   {
      u->coeff2 = 1.0;
   }
   else if( (u->value1 - u->value2) > 0)
   {
      u->coeff2 = 1.0;
   }
   else
   ...
   /* Use backward Euler to propagate solution to second value*/
   (u->value2) = 1./(1. + (-u->coeff2)*dt)*(u->value1);
            ...
            braid_StepStatus status)
{
   ...
   dt = (tstop - tstart)/2.0;
   (r->value1) = (1. + (-ustop->coeff1)*dt)*(ustop->value1) - (r->value1);
   (r->value2) = (1. + (-ustop->coeff2)*dt)*(ustop->value2) - (r->value2);
...
my_Sum(braid_App     app,
       ...
       braid_Vector  y)
{
   (y->value1) = alpha*(x->value1) + beta*(y->value1);
   (y->value2) = alpha*(x->value2) + beta*(y->value2);
               ...
               double       *norm_ptr)
{
   ...
   dot = (u->value1)*(u->value1) + (u->value2)*(u->value2);
   *norm_ptr = sqrt(dot);
