          ...
          braid_StepStatus status)
{
   ...
   if(u->size < 3)
   {
      printf("You've coarsened down to less than 3 points in space.  Error.\n");
      abort();
   }
...
int main (int argc, char *argv[])
{
   ...
   while (arg_index < argc)
   {
      if ( strcmp(argv[arg_index], "-help") == 0 )
      {
         ...
         if ( myid == 0 )
         {
            ...
            printf("  -tfinal <tfinal>     : set the end time for the simulation (default 5.0)\n"); 
   ...
   // Compute the correct max wavespeed for Burger's.  If problem == 0 (i.e., this
   // is linear advection), then 'a' already has the right value. 
   if(problem == 1)
   {
      if(xLeft > xRight)
         a = xLeft;
   ...
   printf(" For Burgers' equation, a = max(xLeft, xRight)\n"); 
   printf(" For Advection a = value set at command line\n\n"); 
