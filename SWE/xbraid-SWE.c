        ...
        int size)
{
    ...
    ((*u)->uhm) = (double *) malloc((size)*sizeof(double));
    ((*u)->hm) = (double *) malloc(size*sizeof(double));
}
        ...
        braid_StepStatus status)
{
    ...
    double dx = (app->xstop - app->xstart)/(ustop->size-1.);
    double dt = (tstop - tstart);

    double* h_old = (double *) malloc(size*sizeof(double));
    double* uh_old =  (double *) malloc(size*sizeof(double));

    for (i=0;i<size;i++)
    {
        h_old[i] = (u->h)[i];
        uh_old[i] = (u->uh)[i];
//	printf("%f %f\n", uh_old[i]/h_old[i], h_old[i] + .5*g*h_old[i]/(uh_old[i]/h_old[i]));
    ...
    for ( i = 0; i < size - 1; i++ )
    {
        (u->hm)[i] = ( (u->h)[i] + (u->h)[i+1] ) / 2.0 
            - ( dt / 2.0 ) * ( (u->uh)[i+1] - (u->uh)[i] ) / dx;
    }
    for ( i = 0; i < size - 1; i++ )
    {
        (u->uhm)[i] = ( (u->uh)[i] + (u->uh)[i+1] ) / 2.0 
            - ( dt / 2.0 ) * ( 
                    (u->uh)[i+1] * (u->uh)[i+1] / (u->h)[i+1] + 0.5 * g * (u->h)[i+1] * (u->h)[i+1]
                    - (u->uh)[i] * (u->uh)[i]  / (u->h)[i] - 0.5 * g * (u->h)[i] * (u->h)[i] ) / dx;
    }
    /*
    ...
    {
        (u->h)[i] = (u->h)[i] 
            - dt * ( (u->uhm)[i] - (u->uhm)[i-1] ) / dx;
    }
    for ( i = 1; i < size; i++ )
    {
        (u->uh)[i] = (u->uh)[i] 
            - dt * ( 
                    (u->uhm)[i] * (u->uhm)[i] / (u->hm)[i] + 0.5 * g * (u->hm)[i] * (u->hm)[i]
                    - (u->uhm)[i-1] * (u->uhm)[i-1] / (u->hm)[i-1] - 0.5 *g * (u->hm)[i-1] * (u->hm)[i-1]) / dx;
    }
    /*
    ...
    //(u->h)[0] = h_old[size-2];
    //(u->h)[size-1] = h_old[1];
    //(u->uh)[0] = uh_old[size-2];
    //(u->uh)[size-1] = uh_old[1];
    ...
    // Free BC:
    u->h[0] = h_old[1];
    u->h[size-1] = h_old[size-2];
    u->uh[0] = uh_old[1];
    u->uh[size-1] = uh_old[size-2];
    return 0;
}
...
{
    int size = (app->nspace);
    *size_ptr = (4*size+1)*sizeof(double);
    return 0;
}
        ...
        braid_BufferStatus bstatus)
{
    ...
    for (int i = 0; i < size; i++) {
        ...
        dbuffer[2*size+i+1] = (u->hm)[i];
        dbuffer[3*size+i+1] = (u->uhm)[i];
    }
    braid_BufferStatusSetSize( bstatus, (4*size+1)*sizeof(double) );
        ...
        braid_BufferStatus bstatus)
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        (u->hm)[i] = dbuffer[2*size+i+1];
        (u->uhm)[i] = dbuffer[3*size+i+1];
    }
    *u_ptr = u;
        ...
        braid_CoarsenRefStatus status)
{
    ...
    if( level < floor(log2(app->nspace)) - 1 )
    {
        ...
        (v->hm) = (double *) malloc(csize*sizeof(double));
        (v->uhm) = (double *) malloc(csize*sizeof(double));
        ...
        braid_CoarsenRefStatus status)
{
    ...
    if( level < floor(log2(app->nspace)) - 1 )
    {
        ...
        (v->hm) = (double *) malloc(fsize*sizeof(double));
        (v->uhm) = (double *) malloc(fsize*sizeof(double));
...
//            my_BufUnpack, my_Coarsen, my_Interp, my_Residual, my_Step);
   ...
   braid_TestCoarsenRefine(app, comm, stdout, 0.0, 0.1, 0.2, my_Init,
         my_Access, my_Free, my_Clone, my_Sum, my_SpatialNorm, my_Coarsen,  
         my_Interp);
